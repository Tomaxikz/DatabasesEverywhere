...
pub(crate) async fn update(
    ...
    instances::spawn_owned_mutation_task(async move {
        ...
        let result = async {
            ...
            state
                .docker
                .pull_image_with_progress(&request.image, &move |event| {
            ...
            let (image_id, version) = state
                .docker
                .probe_image_version(pool.protocol, &request.image)
                .await
            ...
            lifecycle::activate_locked(&state, &mut pool, false)
                .await
                .map_err(|error| ApiError::Runtime(error.to_string()))?;
            ...
            if was_running {
                ...
                lifecycle::honor_stop(&state, &mut pool)
                    .await
                    .map_err(|error| ApiError::Runtime(error.to_string()))?;
            }
