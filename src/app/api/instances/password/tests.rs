...
fn postgres_rotation_hardens_password_enforcement_after_setting_the_secret() {
    assert!(needs_postgres_hardening(
        Protocol::Postgres,
        Some(&SecretString::from("replacement-password"))
    ));
    assert!(!needs_postgres_hardening(Protocol::Postgres, None));
    assert!(!needs_postgres_hardening(
        Protocol::Mysql,
        Some(&SecretString::from("replacement-password"))
    ));
...
fn password_commit_resolution_accepts_only_exact_previous_or_intended_metadata() {
    ...
    assert!(matches!(
        classify_password_commit(intended.clone(), &previous, &intended),
        PasswordMetadataCommitResolution::Committed
    ));
    assert!(matches!(
        classify_password_commit(previous.clone(), &previous, &intended),
        PasswordMetadataCommitResolution::Previous
    ));

    let mut divergent = intended.clone();
    divergent.status = InstanceStatus::Failed;
    match classify_password_commit(divergent.clone(), &previous, &intended) {
...
fn test_metadata(protocol: Protocol) -> InstanceMetadata {
    let mut metadata = crate::instances::test_support::metadata("inst_password_test", protocol);
    ...
    metadata.runtime.container_name = "dbe-test".to_string();
    metadata.database.name = "app_db".to_string();
    metadata.database.username = "app_user".to_string();
    metadata
}
