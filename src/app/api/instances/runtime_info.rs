...
pub(super) async fn enrich_runtime_info(
    ...
    let current = match inspection.as_ref() {
        ...
        _ => state
            .docker
            .container_image(metadata.protocol, metadata.runtime_id())
            .await
            .ok()
            .flatten(),
    };
...
async fn enrich_shared_runtime_info(
    state: &AppState,
    mut metadata: InstanceMetadata,
) -> InstanceMetadata {
        ...
        Ok(Some(_)) => {
            metadata.database_version = Some(InstanceDatabaseVersion {
                ...
                error: Some(PublicDiagnostic::public(
                    "shared_runtime_mismatch",
                    "the shared database runtime does not match this tenant",
                )),
            });
        }
        Ok(None) => {
            metadata.database_version = Some(InstanceDatabaseVersion {
                current: None,
                error: Some(PublicDiagnostic::public(
                    "shared_runtime_missing",
                    "the shared database runtime record is missing",
                )),
            });
        }
        Err(error) => {
            metadata.database_version = Some(InstanceDatabaseVersion {
                ...
                error: Some(PublicDiagnostic::internal(
                    "shared runtime information lookup",
                    error,
                )),
            });
        }
...
pub(super) async fn inspect_live_instance(
    ...
    Some(
        state
            .instance_runtime_cache
            .inspect_instance(&state.docker, metadata.protocol, metadata.runtime_id())
            .await,
    )
}
