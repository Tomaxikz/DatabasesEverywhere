...
pub(super) struct MongoArchiveCatalog {
    ...
    pub(super) shared_issue: Option<MongoSharedIssue>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum MongoSharedIssue {
    SystemDatabase,
    InvalidDatabase,
    SystemCollection,
    InvalidCollection,
    UnsafeCollectionMetadata,
}
...
fn inspect_native_archive<R: Read>(
    ...
    loop {
        ...
        shared_issue = shared_issue.or_else(|| shared_metadata_issue(&metadata));
...
fn parse_metadata(bytes: &[u8]) -> Result<MongoMetadata<'_>, InspectionError> {
    ...
    for element in metadata.iter_elements() {
        ...
        match element.key() {
            ...
            "metadata" => {
                set_required_once(&mut collection_metadata, value.as_str(), MALFORMED)?;
            }
            "type" => set_required_once(&mut kind, value.as_str(), MALFORMED)?,
            _ => {}
        }
    }
    Ok(MongoMetadata {
        database: database.ok_or(InspectionError::Invalid(MALFORMED))?,
        collection: collection.ok_or(InspectionError::Invalid(MALFORMED))?,
        metadata: collection_metadata,
        kind,
    })
}

fn shared_metadata_issue(metadata: &MongoMetadata<'_>) -> Option<MongoSharedIssue> {
    if is_system_database(metadata.database) {
        return Some(MongoSharedIssue::SystemDatabase);
    }
    if !safe_source_database(metadata.database) {
        return Some(MongoSharedIssue::InvalidDatabase);
    }
    if metadata.collection.starts_with("system.")
        || metadata.collection == "$cmd"
        || metadata.collection.contains('$')
    {
    ...
    {
        return Some(MongoSharedIssue::InvalidCollection);
    }
    if metadata.kind.is_some_and(|kind| kind != "collection") {
        return Some(MongoSharedIssue::UnsafeCollectionMetadata);
    }
    let Some(encoded) = metadata.metadata else {
        return Some(MongoSharedIssue::UnsafeCollectionMetadata);
    };
    let Ok(value) = serde_json::from_str::<serde_json::Value>(encoded) else {
        return Some(MongoSharedIssue::UnsafeCollectionMetadata);
    };
    if contains_unsafe_mongo_option(&value) {
        Some(MongoSharedIssue::UnsafeCollectionMetadata)
    } else {
        None
    }
}

fn contains_unsafe_mongo_option(root: &serde_json::Value) -> bool {
...
fn safe_source_database(database: &str) -> bool {
    crate::api::import_export::remote::validate_mongodb_database_name(
