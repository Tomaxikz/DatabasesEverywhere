...
use crate::{
    api::http::response::ApiError,
    shared::{ids::portable_identifier, protocol::Protocol},
};
...
fn inspect_sql_source(
    ...
    scan_sql_source(source, format, protocol, deadline, &mut inspect)
}

fn scan_sql_source<E, F>(
    source: &mut File,
    format: DumpArchiveFormat,
    protocol: Protocol,
    deadline: Instant,
    ...
    F: FnMut(&mut dyn Read) -> Result<(), E>,
{
    ...
    match format {
        ...
        DumpArchiveFormat::Tar => scan_sql_tar(source, false, protocol, deadline, scan),
        DumpArchiveFormat::TarGzip => scan_sql_tar(source, true, protocol, deadline, scan),
        DumpArchiveFormat::Zip => scan_sql_zip(source, protocol, deadline, scan),
    }
}
    ...
    F: FnMut(&mut dyn Read) -> Result<(), E>,
{
    if gzipped {
        let decoder = flate2::read::MultiGzDecoder::new(source);
        scan_sql_tar_reader(decoder, protocol, deadline, scan)
    } else {
        scan_sql_tar_reader(source, protocol, deadline, scan)
    }
}

fn scan_sql_tar_reader<R: Read, E, F>(
    reader: R,
    protocol: Protocol,
    deadline: Instant,
    scan: &mut F,
