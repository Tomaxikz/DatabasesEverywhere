...
const MAX_CAPTURED_COMMENT_BYTES: usize = 64 * 1024;
...
pub(super) fn inspect_sql_reader<R: Read>(
    ...
    let mut observer = CatalogObserver { catalog };
    scan_sql_reader(reader, protocol, &mut observer)
}

trait SqlObserver {
    type Error: From<InspectionError>;

    fn comment(&mut self, _comment: &SqlComment) -> Result<(), Self::Error> {
        Ok(())
    }

    fn token(&mut self, _token: &SqlToken) -> Result<(), Self::Error> {
        Ok(())
    }

    fn statement(&mut self, protocol: Protocol, statement: &Statement) -> Result<(), Self::Error>;
}

fn scan_sql_reader<R: Read, O: SqlObserver>(
    reader: R,
    protocol: Protocol,
    observer: &mut O,
) -> Result<(), O::Error> {
    ...
    while let Some(event) = lexer.next_event()? {
        match event {
            LexEvent::Comment(comment) => observer.comment(&comment)?,
            LexEvent::Token(SqlToken::Semicolon) => {
                let copy_data = statement.finish(protocol, observer)?;
                if copy_data {
    ...
    statement.finish(protocol, observer)?;
    Ok(())
}

struct CatalogObserver<'a> {
    catalog: &'a mut CatalogBuilder,
}

impl SqlObserver for CatalogObserver<'_> {
    type Error = InspectionError;
    ...
    fn statement(&mut self, _protocol: Protocol, statement: &Statement) -> Result<(), Self::Error> {
        statement.record_catalog(self.catalog)
    }
}
...
impl<R: Read> SqlLexer<R> {
    ...
    fn next_event(&mut self) -> Result<Option<LexEvent>, InspectionError> {
        loop {
            ...
            if matches!(byte, b'"' | b'`') {
                ...
                return Ok(Some(LexEvent::Token(identifier.map_or(
                    SqlToken::OversizedIdentifier,
                    SqlToken::QuotedIdentifier,
                ))));
            }
            if byte == b'$'
    ...
    fn read_line_comment(&mut self) -> Result<SqlComment, InspectionError> {
    ...
    fn read_block_comment(&mut self) -> Result<SqlComment, InspectionError> {
        let mut comment = Vec::with_capacity(128);
        let mut truncated = false;
...
impl Statement {
    fn push(&mut self, token: SqlToken) {
        match &token {
            ...
            SqlToken::QuotedIdentifier(_) | SqlToken::Other => {}
    ...
    fn finish<O: SqlObserver>(
        ...
        observer.statement(protocol, self)?;
        let first_is_copy =
    ...
    fn record_catalog(&self, catalog: &mut CatalogBuilder) -> Result<(), InspectionError> {
...
fn identifier_at(tokens: &[SqlToken], index: usize) -> Option<&str> {
    match tokens.get(index) {
        Some(SqlToken::Identifier(identifier) | SqlToken::QuotedIdentifier(identifier)) => {
            Some(identifier)
        }
        _ => None,
    }
