...
pub(super) fn wipe_logical_script(
    ...
    let script = match metadata.protocol {
        Protocol::Postgres if metadata.deployment_mode == DeploymentMode::Shared => r#"set -eu
...
"#
        .to_string(),
        Protocol::Mariadb if metadata.deployment_mode == DeploymentMode::Shared => r#"set -eu
{
...
pub(super) enum ImportConnection {
    LocalSocket,
    PoolLoopback,
}
