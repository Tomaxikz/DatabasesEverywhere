...
pub(super) async fn export_physical_archive(
    ...
    reject_shared_physical(&metadata)?;
...
async fn restore_archive(
    ...
    let replacement = match prepare_restore(paths.clone(), artifact_path, max_extracted_bytes).await
    {
    ...
    let validation = async {
        restore_credentials(&metadata, &paths.data).await?;
...
fn reject_shared_physical(metadata: &InstanceMetadata) -> Result<(), ApiError> {
    if metadata.deployment_mode == DeploymentMode::Shared {
        return Err(ApiError::BadRequest(
            "physical data-directory backup and restore is not allowed for a shared tenant"
                .to_string(),
        ));
    }
    Ok(())
...
mod transaction_tests {
    ...
    async fn restored_resp_acl_uses_current_credentials_and_preserves_data() {
        for protocol in [Protocol::Redis, Protocol::Valkey] {
            ...
            let mut metadata = crate::instances::test_support::metadata("tenant", protocol);
            ...
            restore_credentials(&metadata, dir.path()).await.unwrap();
            let acl = std::fs::read_to_string(dir.path().join("users.acl")).unwrap();
            assert!(!acl.contains("archived credential"));
            assert!(
                acl.contains(&crate::protocols::redis::password_route_sha256(
            ...
            assert!(restore_credentials(&metadata, dir.path()).await.is_err());
        }
    }
