...
pub(super) fn spawn_export_supervisor(
    ...
    tokio::spawn(async move {
        ...
        let execution_cost = if metadata.deployment_mode == DeploymentMode::Shared {
            let _runtime_operation = state.instance_locks.lock(metadata.runtime_id()).await;
            estimate_export_cost(&state, &metadata, &options).await
        } else {
            estimate_export_cost(&state, &metadata, &options).await
        };
...
pub(super) fn spawn_import_supervisor(
    ...
    tokio::spawn(async move {
        ...
        let execution_cost = if metadata.deployment_mode == DeploymentMode::Shared {
            let _runtime_operation = state.instance_locks.lock(metadata.runtime_id()).await;
            estimate_import_cost(&state, &metadata, &options).await
        } else {
            estimate_import_cost(&state, &metadata, &options).await
        };
        let execution = match state
...
enum JobBeginOutcome {
    Running,
    Closed,
    Uncertain,
}

async fn begin_import_export_job(state: &AppState, job_id: &str) -> JobBeginOutcome {
