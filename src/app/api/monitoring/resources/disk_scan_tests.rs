...
use super::disk_scan::directory_size;
...
fn disk_scan_counts_regular_files_without_following_symlinks() {
    ...
    assert_eq!(directory_size(&data, Duration::from_secs(1)).unwrap(), 18);
}
...
fn disk_scan_rejects_a_symlink_root() {
    ...
    assert!(directory_size(&linked, Duration::from_secs(1)).is_err());
}
