...
use crate::{
    api::http::{policy::websocket_token, response::ApiError, router::AppState},
    auth::{api_token::AcceptedApiToken, jwt},
    shared::{hex::encode_lower, time::now_unix},
};
    ...
    fn audit_label(&self) -> String {
        match self {
            Self::Authenticated {
                ...
                format!("{}-peer:{}", kind.as_str(), encode_lower(fingerprint))
            }
