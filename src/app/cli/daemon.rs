...
pub(super) async fn run_daemon(config_path: PathBuf) -> anyhow::Result<()> {
    ...
    let migration_recovery = placements
        ...
        .recover_unfinished()
        .await
        .context("failed to classify interrupted deployment migrations")?;
    if migration_recovery.failed_before_mutation > 0
        || migration_recovery.rollback_pending > 0
        || migration_recovery.cleanup_pending > 0
        || migration_recovery.already_pending > 0
    {
        tracing::warn!(
            failed_before_mutation = migration_recovery.failed_before_mutation,
            rollback_pending = migration_recovery.rollback_pending,
            cleanup_pending = migration_recovery.cleanup_pending,
            manual_or_already_pending = migration_recovery.already_pending,
            "classified interrupted deployment migrations before publishing database routes"
        );
    }
    ...
    let state = AppState::new(AppStateData {
        ...
        instance_locks: instance_locks.clone(),
        docker: docker.clone(),
    ...
    // gateway listeners remain closed until the later tenant-security pass.
    restore_disk_limits(&config, &manager, &docker, &disk_limiter)
        .await
        .context("failed to reapply instance disk limits before recovery")?;
    ...
    let orphan_recovery = recover_orphan_reservations(&state)
        .await
        .context("failed to recover interrupted shared tenant creations")?;
    if orphan_recovery.checked > 0 {
        tracing::info!(
            checked = orphan_recovery.checked,
            cleaned = orphan_recovery.cleaned,
            quarantined = orphan_recovery.quarantined,
    ...
    if removed_empty_shared_runtimes > 0 {
        tracing::info!(
            ...
            "finished interrupted cleanup for empty shared pools"
        );
    }
    // Orphan/deletion recovery can release reservations. Tighten the shared
    // aggregate again around the surviving tenants after those durable
    // cleanup commits.
    restore_shared_limits(&state, &disk_limiter)
        .await
        .context("failed to reapply shared pool limits")?;
    tracing::info!("dedicated instance and shared pool limits reconciled");
    let reconcile_summary = reconcile::reconcile_all(&manager, &docker)
        .await
        .context("failed to reconcile instance metadata")?;
    let shared_reconcile = reconcile_shared_runtimes(&state)
        .await
        .context("failed to reconcile shared pool metadata")?;
    tracing::info!(
        phase = "instance_reconciliation",
        dedicated_checked = reconcile_summary.checked,
        dedicated_booting = reconcile_summary.booting,
        dedicated_running = reconcile_summary.running,
        dedicated_stopped = reconcile_summary.stopped,
        dedicated_failed = reconcile_summary.failed,
        dedicated_quarantined = reconcile_summary.quarantined,
        shared_pools_checked = shared_reconcile.checked,
        shared_pools_booting = shared_reconcile.booting,
        shared_pools_running = shared_reconcile.running,
        shared_pools_stopped = shared_reconcile.stopped,
        shared_pools_failed = shared_reconcile.failed,
        shared_pools_quarantined = shared_reconcile.quarantined,
