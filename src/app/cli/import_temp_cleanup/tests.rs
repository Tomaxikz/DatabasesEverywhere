...
const SIMPLE_RESTORE_ID: &str = "00000000000040008000000000000004";
...
#[test]
fn shared_restore_cleanup_removes_only_exact_private_sandboxes() {
    let temp = tempfile::tempdir().unwrap();
    let root = staging_root(&temp);
    let sandbox = root.join(format!(".dbe-shared-restore-{SIMPLE_RESTORE_ID}"));
    fs::create_dir(&sandbox).unwrap();
    fs::write(sandbox.join("input"), b"snapshot").unwrap();

    let lookalike = root.join(format!(".dbe-shared-restore-{SIMPLE_RESTORE_ID}-extra"));
    fs::create_dir(&lookalike).unwrap();
    fs::write(lookalike.join("input"), b"preserve").unwrap();
    let victim = temp.path().join("victim");
    fs::create_dir(&victim).unwrap();
    fs::write(victim.join("input"), b"untouched").unwrap();
    let linked = root.join(".dbe-shared-restore-00000000000040008000000000000005");
    symlink(&victim, &linked).unwrap();

    let summary = cleanup_shared_restore_root(&root).unwrap();
    ...
    assert_eq!(fs::read(victim.join("input")).unwrap(), b"untouched");
}

#[test]
fn malformed_shared_restore_sandbox_is_retained() {
    let temp = tempfile::tempdir().unwrap();
    let root = staging_root(&temp);
    let sandbox = root.join(format!(".dbe-shared-restore-{SIMPLE_RESTORE_ID}"));
    fs::create_dir(&sandbox).unwrap();
    fs::write(sandbox.join("input"), b"snapshot").unwrap();
    fs::write(sandbox.join("unexpected"), b"preserve").unwrap();

    assert!(cleanup_shared_restore_root(&root).is_err());
    assert!(sandbox.exists());
}
