...
/// Removes private shared-restore snapshots only after helper containers have
/// been reconciled. Deleting these paths earlier would not revoke an existing
/// bind mount from a stale helper process.
pub(super) async fn cleanup_shared_restore_sandboxes(
    tmp_root: &Path,
) -> anyhow::Result<ImportTempCleanupSummary> {
    let root = tmp_root.join("import-export");
    tokio::task::spawn_blocking(move || cleanup_shared_restore_root(&root))
        .await
...
fn is_shared_restore_sandbox(name: &str) -> bool {
    let Some(id) = name.strip_prefix(".dbe-shared-restore-") else {
        return false;
    };
    id.len() == 32
        && uuid::Uuid::parse_str(id)
            .is_ok_and(|parsed| parsed.get_version_num() == 4 && parsed.simple().to_string() == id)
}

fn validate_shared_restore_sandbox(root: &Path) -> anyhow::Result<()> {
    let mut entries = fs::read_dir(root)
        .with_context(|| format!("failed to scan shared restore sandbox {}", root.display()))?;
    let Some(entry) = entries
        .next()
        .transpose()
        .with_context(|| format!("failed to read shared restore sandbox {}", root.display()))?
    else {
        return Ok(());
    };
    let path = entry.path();
    let metadata = fs::symlink_metadata(&path)
        .with_context(|| format!("failed to inspect shared restore input {}", path.display()))?;
    if entry.file_name() != "input" || metadata.file_type().is_symlink() || !metadata.is_file() {
        bail!(
            "shared restore sandbox {} contains an unexpected entry",
            root.display()
        );
    }
    if entries.next().transpose()?.is_some() {
        bail!(
            "shared restore sandbox {} contains multiple entries",
            root.display()
        );
    }
    Ok(())
