...
pub(super) async fn finish_runtime_boot(state: AppState) {
    ...
    if let Err(error) = start_shared_runtimes(&state).await {
        tracing::error!(
            %error,
    ...
    if resumed_shared_deletions > 0 {
        tracing::info!(
            ...
            "finished shared tenant deletions after their physical pools became available"
        );
    }
    ...
    if compatibility.failed == 0 && shared_compatibility.failed == 0 {
        tracing::info!(
            ...
            shared_pools_probed = shared_compatibility.probed,
        ...
        tracing::warn!(
            ...
            shared_attestations_reused = shared_compatibility.reused,
            shared_pools_probed = shared_compatibility.probed,
            shared_pools_failed = shared_compatibility.failed,
            "configured image reconciliation completed with isolated instance failures"
        );
    }
    let shared_tenants = match reconcile_shared_tenants(&state).await {
        Ok(summary) => summary,
        Err(error) => {
            tracing::error!(
                %error,
                "shared tenant state reconciliation failed; database gateways remain closed"
            );
            state
                .gateway_supervisor
                .fail_and_stop("shared tenant state reconciliation failed");
            return;
        }
    };
    tracing::info!(
        checked = shared_tenants.checked,
        opened = shared_tenants.opened,
        fenced = shared_tenants.fenced,
        quarantined = shared_tenants.quarantined,
        pools_contained = shared_tenants.pools_contained,
        "reapplied durable shared tenant credentials, quotas, and access state before gateway publication"
    );
    if !state.import_export_jobs.is_accepting() {
        return;
    }
    ...
    if shared_disk_failures == 0 {
        ...
        tracing::warn!(
            shared_pools_failed = shared_disk_failures,
            "shared tenant disk quota initialization completed fail-closed; affected tenant routes remain fenced while background sampling retries"
        );
    }
    if !state.import_export_jobs.is_accepting() {
        return;
    ...
    let active_migration_fences =
        match crate::api::instances::fence_active_deployment_migration_routes(&state).await {
            ...
            Err(error) => {
                tracing::error!(
                    %error,
                    "could not enumerate active deployment migrations before gateway publication; database gateways remain closed"
                );
                state
                    .gateway_supervisor
                    .fail_and_stop("deployment migration route fencing failed");
                return;
            }
        };
    if active_migration_fences > 0 {
        tracing::warn!(
            active_migration_fences,
            "reasserted active deployment migration route fences before gateway publication"
        );
    }
