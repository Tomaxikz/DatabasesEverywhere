...
impl DockerRuntime {
    /// Streams a private regular host file to the stdin of a shell command.
    ///
    ...
    /// Streams a shell command's stdout to a new private regular host file.
    ///
    ...
    async fn run_streaming_exec(
        ...
        if result.is_err() && active.load(Ordering::Acquire) && recovery.restarts_runtime() {
    ...
    async fn stream_exec(
        ...
        match &direction {
            ExecStreamDirection::Input {
                ...
                let (mut file, expected_bytes) = open_private_input(path, *max_bytes)?;
                if let Some(expected_sha256) = expected_sha256 {
                    verify_private_input(&mut file, expected_bytes, *max_bytes, *expected_sha256)
                        .await
                        .map_err(|source| DockerError::ExecStreamIo {
                            direction: "input verification",
                            path: path.display().to_string(),
                            source,
                        })?;
                }
                input_file = Some((file, expected_bytes));
            }
        ...
        // Once start is attempted, Docker has no API for reliably killing one
        // exec process. Dedicated recovery restarts the runtime; shared callers
        // must terminate and fence only this tenant.
...
pub(super) async fn verify_private_input<F>(
    file: &mut F,
    expected_bytes: u64,
    max_bytes: u64,
...
where
    F: AsyncRead + tokio::io::AsyncSeek + Unpin,
{
    ...
    loop {
        ...
        read_bytes = read_bytes
            .checked_add(read as u64)
            .ok_or_else(|| IoError::new(ErrorKind::InvalidData, "exec input size overflow"))?;
        if read_bytes > max_bytes || read_bytes > expected_bytes {
            return Err(IoError::new(
                ErrorKind::InvalidData,
                "exec input changed size while its identity was verified",
            ));
        }
        hasher.update(&buffer[..read]);
    }
    if read_bytes != expected_bytes || hasher.finalize().as_slice() != expected_sha256 {
        return Err(IoError::new(
            ErrorKind::InvalidData,
            "exec input no longer matches the inspected artifact",
        ));
    }
    file.rewind().await?;
    Ok(())
...
pub(super) fn open_private_input(
    path: &Path,
    max_bytes: u64,
) -> Result<(tokio::fs::File, u64), DockerError> {
...
mod tests {
    ...
    #[tokio::test]
    async fn verified_input_stays_bound_to_the_opened_file() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("dump.sql");
        let moved = directory.path().join("inspected.sql");
        let inspected = b"SELECT 1;";
        std::fs::write(&path, inspected).unwrap();
        let expected: [u8; 32] = Sha256::digest(inspected).into();
        let (mut opened, size) = open_private_input(&path, 64).unwrap();

        std::fs::rename(&path, &moved).unwrap();
        std::fs::write(&path, b"SELECT malicious();").unwrap();

        verify_private_input(&mut opened, size, 64, expected)
            .await
            .unwrap();
        let mut contents = Vec::new();
        opened.read_to_end(&mut contents).await.unwrap();
        assert_eq!(contents, inspected);

        let (mut replacement, replacement_size) = open_private_input(&path, 64).unwrap();
        let error = verify_private_input(&mut replacement, replacement_size, 64, expected)
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn verified_input_rejects_same_length_content_changes() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("dump.sql");
        std::fs::write(&path, b"safe").unwrap();
        let expected: [u8; 32] = Sha256::digest(b"safe").into();
        let (mut opened, size) = open_private_input(&path, 4).unwrap();
        std::fs::write(&path, b"evil").unwrap();

        let error = verify_private_input(&mut opened, size, 4, expected)
            .await
            .unwrap_err();
        assert_eq!(error.kind(), ErrorKind::InvalidData);
    }
