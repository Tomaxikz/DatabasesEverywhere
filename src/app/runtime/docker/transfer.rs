...
#[cfg(test)]
pub(super) fn extract_one_file<R: Read>(
    reader: R,
    expected_file_name: &str,
    host_path: &Path,
) -> Result<(), IoError> {
    extract_one_file_with_limit(
        reader,
        expected_file_name,
        host_path,
        MAX_CONTAINER_TRANSFER_BYTES,
...
#[cfg(test)]
pub(super) fn extract_one_file_with_limit<R: Read>(
    reader: R,
    expected_file_name: &str,
    host_path: &Path,
    max_bytes: u64,
...
pub(super) fn extract_one_file_until<R: Read>(
    reader: R,
    expected_file_name: &str,
    host_path: &Path,
    max_bytes: u64,
