...
pub fn redact_exact_secrets(value: &str, secrets: &[String]) -> String {
    let mut redacted = redact_connection_url(value);
...
mod tests {
    ...
    #[test]
    fn redacts_exact_runtime_secrets() {
        let secrets = vec![
            "opaque-value".to_string(),
            "opaque-value-without-a-marker".to_string(),
        ];
        let redacted = redact_exact_secrets(
            "client said opaque-value-without-a-marker while PASSWORD=second-secret",
            &secrets,
        );
        assert_eq!(redacted, "client said [redacted] while PASSWORD=[redacted]");
    }
}
