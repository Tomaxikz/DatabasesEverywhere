...
pub struct ApiConfig {
    pub host: String,
    pub port: u16,
    /// Retained only to load v0.6 configuration files. Host headers are not
    /// an authorization boundary and this value has no runtime effect.
    #[serde(skip_serializing)]
    pub trusted_hosts: Vec<String>,
    /// Accepted only so configurations generated by older panels keep loading.
    /// The panel owns the daemon's public address; this value never affects
    /// listener binding, request validation, or URL generation.
    #[serde(skip_serializing)]
    pub fqdn: String,
...
impl Config {
    pub fn protocol_enabled(&self, protocol: Protocol) -> bool {
        match protocol {
            Protocol::Postgres => self.postgres.enabled,
            Protocol::Redis => self.redis.enabled,
            Protocol::Valkey => self.valkey.enabled,
            Protocol::Mariadb => self.mariadb.enabled,
            Protocol::Mysql => self.mysql.enabled,
            Protocol::Mongodb => self.mongodb.enabled,
            Protocol::Clickhouse => self.clickhouse.enabled,
            Protocol::Qdrant => self.qdrant.enabled,
        }
    }
...
impl ImageConfig {
    pub fn configured_for_protocol(&self, protocol: Protocol) -> &str {
        match protocol {
            ...
            Protocol::Redis => &self.redis,
            Protocol::Valkey => &self.valkey,
            Protocol::Mariadb => &self.mariadb,
            Protocol::Mysql => &self.mysql,
            Protocol::Mongodb => &self.mongodb,
            Protocol::Clickhouse => &self.clickhouse,
            Protocol::Qdrant => &self.qdrant,
        }
    }
    ...
    pub fn allowed_for_protocol(&self, protocol: Protocol) -> Vec<&str> {
        ...
        let mut allowed = match protocol {
            Protocol::Postgres => self.allowed.postgres.iter(),
            Protocol::Redis => self.allowed.redis.iter(),
            Protocol::Valkey => self.allowed.valkey.iter(),
            Protocol::Mariadb => self.allowed.mariadb.iter(),
            Protocol::Mysql => self.allowed.mysql.iter(),
            Protocol::Mongodb => self.allowed.mongodb.iter(),
            Protocol::Clickhouse => self.allowed.clickhouse.iter(),
            Protocol::Qdrant => self.allowed.qdrant.iter(),
        }
        .map(String::as_str)
        .collect::<Vec<_>>();
