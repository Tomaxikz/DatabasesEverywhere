...
impl HybridScanPlanner {
    ...
    #[cfg(test)]
    pub(crate) fn config(&self) -> PlannerConfig {
        self.config
    }

    #[cfg(test)]
    pub(crate) fn reconfigure(&mut self, now: PlannerTick, config: PlannerConfig) -> bool {
    ...
    pub(crate) fn remove_target(&mut self, target_id: &str) -> bool {
        self.targets.remove(target_id).is_some()
    }

    pub(crate) fn set_watcher_trusted(
        &mut self,
        target_id: &str,
        trusted: bool,
        now: PlannerTick,
    ...
    #[cfg(test)]
    pub(crate) fn mark_all_overflow(&mut self, now: PlannerTick) -> Vec<(String, u64)> {
    ...
    pub(crate) fn next_candidate(&mut self, now: PlannerTick) -> Option<ScanCandidate> {
        if self.shutdown {
            return None;
        }
    ...
    pub(crate) fn next_wakeup(&self, now: PlannerTick) -> Option<Duration> {
        if self.shutdown {
            return None;
        }
        self.targets
    ...
    #[cfg(test)]
    pub(crate) fn target_status(&self, target_id: &str) -> Option<PlannerTargetStatus> {
