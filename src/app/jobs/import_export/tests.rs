...
fn bounded_physical_extraction_rejects_before_oversized_file_is_written() {
    ...
    let error =
        extract_bounded_archive_blocking(&archive_path, &target, "data", limit).unwrap_err();
...
#[test]
fn physical_backup_rejects_internal_source_symlinks() {
...
#[test]
fn physical_restore_rejects_every_symlink_entry() {
    let dir = tempfile::tempdir().unwrap();
    ...
    let error =
        extract_bounded_archive_blocking(&lexically_safe, &target, "data", MAX_DATA_ARCHIVE_BYTES)
            .unwrap_err();
