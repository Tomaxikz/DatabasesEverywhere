...
pub fn fuse_quota_unsupported(mode: DiskLimitMode) -> bool {
...
mod tests {
    ...
    fn qdrant_requires_native_storage_instead_of_fuse_quota() {
        assert!(fuse_quota_unsupported(DiskLimitMode::FuseQuota));
        assert!(!fuse_quota_unsupported(DiskLimitMode::ProjectQuota));
