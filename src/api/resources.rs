...
pub(crate) async fn resource_report(
    ...
    let scanner_active = soft_enforcement || legacy_qdrant_safety_monitor;
    ...
    let disk_used = reported_disk_used_bytes(scanner.as_ref(), || async {
        state
            .resource_cache
    ...
    let report = ResourceReport {
        ...
        disk: DiskReport {
            ...
            enforcement_strength: if soft_enforcement {
                "soft"
            } else if metadata.limits.disk_enforced {
                "hard"
            } else {
                "none"
            },
            ...
            scanner_restart_blocked: scanner_active.then(|| {
...
async fn reported_disk_used_bytes<F, Fut>(
