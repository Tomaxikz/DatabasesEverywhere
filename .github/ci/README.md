...
[Socket Security GitHub App](https://github.com/apps/socket-security) and grant
access to **Tomaxikz/DatabasesEverywhere** only. Repository YAML cannot grant that
access. This integration needs no workflow API-key secret and does not add a
second CLI scanner. Verify that Socket produces a check on a dependency PR
before making that check required in branch rules. Existing `cargo-audit`,
CodeQL, and Trivy checks stay enabled.
