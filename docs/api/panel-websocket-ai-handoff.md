...
connects to DatabasesEverywhere (DBEV) `0.8.0`, API contract `0.14.0`. It is
...
The runtime implementation and [`api/openapi.yml`](openapi.yml) remain authoritative if a later
...
| --- | --- | --- | --- |
| `/ws/monitoring` | `monitor:read` | JWT allow-list and bound instance generations are applied to every snapshot | Full current authorized snapshot every second |
...
- Every listed ID is cryptographically bound to the instance generation that
  exists when the token is minted. If that instance is deleted or recreated,
  the token cannot observe the replacement; mint a new token after the panel
  authorizes it.
...
The server produces a complete authorized snapshot every second. A snapshot is
split into bounded batches when necessary. Buffer every batch with the same
`sequence`, then atomically replace panel state only after indexes
`0..batch_count-1` are present. It is not a delta. Instances outside the JWT
allow-list never appear.
...
type MonitoringMessage = {
  ...
  instances: Array<{
    ...
    resource_scope: "dedicated_instance" | "shared_tenant";
    protocol: string;
    status: string;
    runtime: string;
    activity: {
      ...
      stats_epoch: string;
      sampled_at_unix: number;
      accepted: { read: number; write: number; ddl: number; other: number };
      rejected: { read: number; write: number; ddl: number; other: number };
      active_connections: number;
      opened_connections: number;
      rx_bytes: number;
      tx_bytes: number;
      cpu_time_micros: number | null;
      peak_query_memory_bytes: number | null;
      sources: {
        connections: "gateway_authenticated_exact" | "unavailable";
        network: "gateway_route_exact";
        operations:
          | "gateway_protocol_observed"
          | "engine_query_log_observed"
          | "unavailable";
        cpu_time: "engine_query_observed" | "unavailable";
        peak_query_memory: "engine_query_observed" | "unavailable";
      };
    };
...
- `activity` is cumulative live tenant activity. Draw rates from deltas between
  snapshots and reset the local baseline when `stats_epoch` changes. Load
  durable one-minute chart history from
  `GET /api/instances/{id}/activity/history`; never derive durable totals from
  the WebSocket alone. In history, `operations_observed: false` means the
  source was unavailable; it is not a measured zero.
- `resources.cpu.usage_percent` is expressed in percentage points. Render
  `11.0` as `11.0%`; do not divide or multiply it by 100.
- DBEV follows Calagopus wings-rs: it obtains one non-streaming Docker counter
  snapshot per physical runtime per second and calculates CPU from container
...
- For `deployment_mode: "shared"`, `resources.cpu.usage_percent`,
  `resources.memory.usage_bytes`, and `resources.memory.limit_bytes` are
  `null`: the runtime cgroup cannot honestly attribute those values to one
  logical tenant. The configured CPU and memory fields inside `resources` are
  tenant reservations against aggregate pool capacity, not per-tenant cgroup
  measurements or ceilings. Render tenant
  disk and network normally. Pool health is administrator-only and must be
  loaded from `/api/admin/shared-pools`; tenant WebSocket messages intentionally
  omit physical pool CPU and memory. Never relabel pool values as tenant usage
  or limits. The nested resource report describes the individual tenant disk
  boundary: hard native project
  quotas report `true`/`hard`, while catalog-measured tenants report
  `false`/`soft`. Do not infer disk enforcement from `deployment_mode`.
...
- Network RX/TX counts bytes observed after DBEV selects a gateway route and at
  its backend boundary. They can include a routed authentication attempt and
  are cumulative only since daemon boot, not billing counters. They reset after
  daemon restart and do not include traffic that bypasses the gateway.
- Operation activity contains only bounded protocol classifications (`read`,
  `write`, `ddl`, `other`), never raw SQL or parameters. Tenant query CPU and
  peak memory are engine observations when non-null, not cgroup CPU/RSS.
  For gateway sources, `accepted` means accepted by DBEV and forwarded to the
  database, not successful execution; `rejected` means DBEV rejected it before
  forwarding. Do not draw database success/error charts from these fields.
  `activity.sources` is included in every WebSocket sample because collector
  availability can change; render `unavailable` as unavailable rather than
  zero. Shared ClickHouse labels operation counts as
  `engine_query_log_observed`; dedicated ClickHouse and an unavailable
  collector return `unavailable`.
...
- `sequence` identifies one complete current-state snapshot. It is not an event
  replay offset and resets on reconnect. Discard incomplete older sequences if
  a newer sequence begins; never interpret one batch as tenant deletion. A
  reconnect simply receives fresh batches for current state.
...
| Monitoring instance list/status | Rebuilt from current daemon/instance state after the next complete batch sequence |
...
available from [`api/openapi.yml`](openapi.yml), reuse the REST `ResourceReport`,
